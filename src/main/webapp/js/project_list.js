...
function setTableBodyRows(result) {//result 为一个json对象
    ...
    for (var i in result) {//循环一次取一行[{"name":"AAA",...},{},{}]
        ...
        tr.append(firstTd);
        tr.append("<td>" + result[i].projectName + "</td>");
        tr.append("<td>" + result[i].projectPrice + "</td>");
        tr.append("<td>" + new Date(result[i].startDate).toLocaleDateString() + "</td>");
